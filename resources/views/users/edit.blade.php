...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
                    ...
                    <div class="col-6">                            
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="role_id" class="form-control">
                                ...
                                <option 
                                value="{{ $role->id }}"
                                {{ (old('role_id') == $role->id) ? 'selected' : in_array($role->id, $userRoles) ? 'selected' : null }}
                                >{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
            ...
            <form action="{{ route('users.passwordChange', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card mt-5">
                    <div class="card-header">{{ __('Change passowrd') }}</div>
            ...
            <form action="{{ route('users.projectsAssign', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card mt-5">
                    <div class="card-header">{{ __('Assign projects') }}</div>
